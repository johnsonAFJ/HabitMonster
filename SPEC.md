...
fills with a piece of furniture every third level from 3, skipping 15 where
the second evolution lands:

| Level | Furniture |
| --- | --- |
| 3 | A picture for the wall |
| 6 | A shelf of books |
| 9 | A floor lamp |
| 12 | A toy chest |
| 18 | A wall clock |
| 21 | A big leafy plant |
| 24 | A trophy, on the toy chest |

The list is `FURNITURE` in `js/monster.js`: explicit, rather than a rule like
"every third level", so nothing can claim a level grants an item that does
not exist. A level that brings one announces it and plays the coin sound
after the level-up sound. Furniture follows the level, so undoing a log can
take a piece back out, and logging again returns it.

Every piece is drawn in code at a fixed spot in `FURNITURE_SPOTS` in
`js/art.js`, chosen so none overlaps another or the monster's box. A PNG in
`assets/furniture/` replaces the drawn version once its id is added to
`FURNITURE_ART`; see `ART_BRIEF.md`. Only listed pieces are fetched, because
asking for files that do not exist costs a request each and the browser logs
every failed image as an error.

## Events at each level

**`EVENTS.md`** in the repo root is the chart of everything that happens and
when. **`events.html`**, linked from the bottom of the app, is the same chart
with his real progress on it: which events are done, what is next, and how
much XP it needs.

Both are built from `levelEvents()` in `js/monster.js`, which reads the same
constants the game runs on, so neither can describe something the game does
not do. The earliest day for each event is found by playing it out through
the engine, not calculated separately. `tests/events.test.js` checks that
every event lands at exactly its level and not a level before, and fails if
`EVENTS.md` has drifted: regenerate it with `npm run events`.
